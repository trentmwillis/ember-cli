...
var reexport     = require('../utilities/deprecated-reexport');
...
Addon.prototype.compileAddon = function(tree) {
  ...
  var captureLegacyReexport = reexport(addonJs, {
    name: this.name,
    outputFile: this.name + '.js'
  });

  var trees = [addonJs, templatesTree, captureLegacyReexport].filter(Boolean);
  return stew.log(mergeTrees(trees));
};
