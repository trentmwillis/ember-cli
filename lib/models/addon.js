...
let addonProto = {
  ...
  */
  jshintAddonTree() {
    ...
    function namespacedTree(treePath) {
      return new Funnel(treePath, {
        srcDir: '/',
        destDir: `${this.name}/${treePath}/`,
      });
    }
    ...
    if (existsSync(appPath)) {
      let appJs = namespacedTree(appPath);
    ...
    if (existsSync(testSupportPath)) {
      let testSupport = namespacedTree(testSupportPath);
    ...
    if (existsSync(addonTestSupportPath)) {
      let addonTestSupport = namespacedTree(addonTestSupportPath);
